...
def structured_payload(value: str) -> object:
    try:
...
async def post_provider_json(
    *,
    ...
    try:
        ...
        raise EvaluationProviderTransportError("timeout") from error
    except httpx.RequestError as error:
        raise EvaluationProviderTransportError("transport_failed") from error
    latency_ms = max(0, round((perf_counter() - started) * 1000))
    if response.status_code == 429:
        raise EvaluationProviderTransportError("rate_limited")
    if response.status_code in {408, 504}:
        raise EvaluationProviderTransportError("timeout")
    if response.status_code >= 500:
        raise EvaluationProviderTransportError("transport_failed")
    if response.status_code >= 400:
        raise EvaluationProviderPermanentError("provider_rejected")
    try:
